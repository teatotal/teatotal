...
| [jobfeed](jobfeed.md)       | A job pool runs work but does not decide which work reaches it: real work has to be polled from a source that may be slow or down, deduplicated so one item never runs twice at once, deliverable by hand as its own tracked run, admitted through a policy gate, and kept as history once it ends. jobfeed is that intake layer over jobloop or jobpool, its gate and outcome-reader the override hooks a consumer's policy fills in. |
...
- `demos/jobfeed-demo.tcl` - a source polled into a two-kind pool, one identity deduped across two polls, a budget deferring work in place, a hand-delivered run waved past the gate, every outcome kept as history.

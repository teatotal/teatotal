...
`running`, `paused`, and `rate_limited` each hold one of the pool's slots; `queued` does not. `rate_limited` is the state for a job that is alive but blocked on something outside the pool: a throttled endpoint, a quota that ran dry until it resets. The pool does not decide when that happens; a worker reports it and reports when it clears.
...
The pool defines a worker-side vocabulary in `::jobloop::worker`: **checkpoint**, **phase**, **progress**, **rate_limited**, **rate_limit_cleared**, **done**, **failed**, each taking the job first. A worker body picks them up with one `namespace path` line, and that line names which twin the body reports to.
...
Cancel and pause are cooperative by design: a worker that never calls `checkpoint` is never interrupted, which is the price of never tearing a coroutine out of its own stack. A per-job deadline is one line away, `after $ms [list $loop cancel $job]`, and `deadman` from this shelf owns the harder case of a subprocess that must die with its whole tree. Destroying the pool cancels its pending timers and deletes its coroutines; that reaping relies on the yield convention above, and a coroutine suspended inside a nested `vwait` of its own cannot be deleted mid-wait. Cancel and drain such a job before destroying the pool, or better, wait the loop's way.
